...
class OSREvaluation:
    ...
    def close_accuracy(self, prediction):
        return (
            np.sum((prediction == self.test_labels) & self.close_samples)
            / self.close_samples_ct
        )
    ...
    def open_detection_indexes(self, scores, thresh):
        ...
        precision, recall, _ = metrics.precision_recall_curve(
            self.close_samples, scores
        )
        aupr_in = metrics.auc(recall, precision)
        precision, recall, _ = metrics.precision_recall_curve(
            np.bitwise_not(self.close_samples), -scores
        )
